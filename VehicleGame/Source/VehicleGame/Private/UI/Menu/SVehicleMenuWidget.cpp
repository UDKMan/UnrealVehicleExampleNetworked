...
void SVehicleMenuWidget::BuildLeftPanel(bool bGoingBack)
{
	if (!CurrentMenu.IsValid())
	{
	...
	SelectedIndex = 0;
	FVehicleMenuItem* FirstMenuItem = CurrentMenu->IsValidIndex(SelectedIndex) ? &(*CurrentMenu)[SelectedIndex] : NULL;
	if (FirstMenuItem != NULL && FirstMenuItem->MenuItemType == EVehicleMenuItemType::Standard)
	{
...
FString SVehicleMenuWidget::GetMenuItemText(int32 Index) const
{
	return (*CurrentMenu)[Index].Text.ToString();
}
